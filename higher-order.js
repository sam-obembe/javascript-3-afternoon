...
//Code Here
let totalPopulation = populations.reduce(callBack,0)
function callBack(acc,curr) { return acc + curr;}
...
let bobsTotal = purchases.map((e) => e.owner ==="Bob"? e.price:0).reduce(function (acc,curr){return acc+curr},0)
//loop through purchases for Bob using the e.owner parameter, if e.owner is Bob, return e.price, purchases.map() forms an array. Reduce that array. That gives us purchases.map(create array of Bobs prices).reduce(sum the array of Bob's prices)
